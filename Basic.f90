...
complex*16 function psy0(x,i)!Gaussian wave packet on 1st state, taking x0, p0, d as arguments
    integer,intent(in)::i; real*8,intent(in)::x
    if(i==1) then
...
real*8 function potential(x,i,j)!Some scattering models take mass as argument
    real*8,intent(in)::x; integer,intent(in)::i,j
    !Scattering model
        !Gaussian well
